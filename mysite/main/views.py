...
from .forms import CreateList, CreatePerson, ShowItems, SetLoginUser

# Create your views here.
def index(request, id):
    login_user = Person.objects.get(upi = MockedLoginUser.objects.get(id=1).upi)
    ls = ToDoList.objects.get(id=id)

    if login_user.upi != ls.user.upi:
...
def home(request):
    ...
    return render(request, "main/home.html", {"list": ls, "login_user": login_user})
...
def createList(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            t = ToDoList(name=form.cleaned_data["name"], user=login_user)
            t.save()
...
def setLoginUser(request):
    login_user = Person.objects.get(upi = MockedLoginUser.objects.get(id=1).upi)

    if request.method == "POST":
        form = SetLoginUser(request.POST)
        if form.is_valid():
            formData = form.cleaned_data
            if Person.objects.filter(upi=formData["upi"]).count() == 0:
                p = Person(name=formData["name"], upi=formData["upi"], isAdmin=False)
                p.save()
            MockedLoginUser.objects.filter(id=1).update(name=formData["name"],upi=formData["upi"])
            return HttpResponseRedirect("setLoginUser")

    currentLoginUPI = MockedLoginUser.objects.get(id=1)
    form = SetLoginUser()

    data = {
        "form": form,
        "name": currentLoginUPI.name,
        "upi": currentLoginUPI.upi,
        "login_user": login_user
    }
    return render(request, "main/setLoginUser.html", data)
